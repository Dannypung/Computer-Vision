...
addtional_train_path_pos = fullfile(data_path, 'extra_faces'); % Additional Positive training examples. 36x36 head crops
...
additional_features_pos_features_pos = get_positive_features( ...
                                addtional_train_path_pos, feature_params );
features_pos = [features_pos;additional_features_pos_features_pos];
...
% copy the negative features
temp_features_neg = features_neg;
% hard negative mining loop
for i=1:200
    score = features_neg * w + b;
    % find the scores greater than hard negative threshhold
    tempindex = find(score >= 0.05);
    if ~isempty(tempindex)
        temp_features_neg = [temp_features_neg;features_neg(tempindex,:)];
    end 
        % combine the features to together and train SVM model again
        features_neg = temp_features_neg;
        X = [features_pos;features_neg]';
        Y = [ones(size(features_pos,1),1); -1*ones(size(features_neg,1),1)];
        [w, b] = vl_svmtrain(X, Y, lambda);
end
