...
features_pos = zeros(num_images,fix(template_size / hog_cell_size)^2 * 31);

for i=1:num_images
    % read in image
    IM = im2single(imread([train_path_pos '/' image_files(i).name]));
    IM = imresize(IM,[template_size,template_size]);
    % generate HOG for train image
    HOG = vl_hog(IM, feature_params.hog_cell_size);
    % store the HOG to the return feature list
    features_pos(i,:)= reshape(HOG,1,fix(template_size / ...
        hog_cell_size)^2 * 31);
end
