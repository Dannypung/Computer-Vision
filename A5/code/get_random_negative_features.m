...
% calculate how many crop images to generate for per train image
cropPerImage = ceil(num_samples/num_images);

features_neg = zeros(cropPerImage*num_images,fix(...
    feature_params.template_size / feature_params.hog_cell_size)^2 * 31);
% index counter for feature list
...
for i=1:num_images
    ...
    for j=1:cropPerImage
        ...
        % generate HOG for the cropped image part
        HOG = vl_hog(im2single(cropImage),feature_params.hog_cell_size);
        % store HOG to returned feature list
